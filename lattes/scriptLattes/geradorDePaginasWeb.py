...
class GeradorDePaginasWeb:
	...
	def gerarPaginaPrincipal(self):
		nomeGrupo = self.grupo.obterParametro('global-nome_do_grupo').decode("utf8")
                nomeGrupo = "UFSM - An&aacutelise do Lattes"
		...
		s+= '</head> \n \
        <body onload="initialize()" onunload="GUnload()"> <div class="container"><div class="row">  \
        <div class="col-md-12"> <h2> '+nomeGrupo+'</h2></div></div><div class="row"><div class="col-md-12">'
	...
	def gerarPaginaDeProducoes(self, listaCompleta, tituloPagina, prefixo, ris=False):
		...
		if len(keys)>0: # apenas geramos páginas web para lista com pelo menos 1 elemento
			...
			for ano in keys:
				...
				s+= '<div class="row"><h3 class="year">'+anoRotulo+'</h3> <table>'
				...
				for index in range(0, len(elementos)):
					...
					if numeroDeItem%maxElementos==0 or numeroDeItem==numeroTotalDeProducoes:
						st = self.paginaTop()
						st+= '\n<div class="row"><h3>'+tituloPagina.decode("utf8")+'</h3> <br> <img src="'+prefixo+'.png"> <br>'
	...
	def paginaTop(self, cabecalho=''):
        ...
        <body><div class="container"> <div class="row"> <button class="btn btn-primary" onClick="history.go(-1)">Voltar</button> \
        <h2> '+nomeGrupo+'</h2> </div>'
		return s
	...
	def formatarTotaisQualis(self, qtd):
		...
		st+= '<li> Publica&ccedil;&atilde;o para a qual o nome exato do Qualis foi identificado: <span class="text-success"><b>Qualis &lt;estrato&gt;</b></span>'
		st+= '<li> Publica&ccedil;&atilde;o para a qual um nome similar (n&atilde;o exato) do Qualis foi identificado: <span class="text-warning"><b>Qualis &lt;estrato&gt;</b></span> (nome similar)'
		st+= '<li> Publica&ccedil;&atilde;o para a qual nenhum nome do Qualis foi identificado: <span class="text-error"><b>Qualis n&atilde;o identificado</b></span> (nome usado na busca)'
		st+= '</ul>'
		return st
...
def formataQualis(qualis, qualissimilar):
	...
	if not qualis==None:
		...
		if qualis=='Qualis nao identificado':
			...
			s += '<span class="text-error"><b>Qualis: N&atilde;o identificado</b></span> ('+qualissimilar+')'
		else:
			if qualissimilar=='':
				# Casamento perfeito - imprime em verde
				s += '<span class="text-success"><b>Qualis: ' + qualis + '</b></span>'	
			else:
				# Similar - imprime em laranja
				s += '<span class="text-warning"><b>Qualis: ' + qualis + '</b></span> ('+qualissimilar+')' 
	return s
